  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {dueCount > 0 && (
          <div className="flex items-center gap-2 px-3 py-1.5 bg-blue-950/40 border border-blue-800/60 rounded-xl">
            <AlertTriangle className="w-4 h-4 text-blue-400" />
            <span className="text-xs text-blue-300 font-semibold">
              {dueCount} unit{dueCount !== 1 ? 's' : ''} due for service
            </span>
          </div>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="p-12 text-center bg-slate-900/60 border border-slate-800 rounded-2xl space-y-3">
          ...
          <p className="text-sm text-slate-300 font-semibold">All aircon units are up to date!</p>
          <p className="text-xs text-slate-500">No units due for 3-month service.</p>
        </div>
      ) : (
