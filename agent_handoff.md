...
- **Icons:** Lucide React
- **PWA:** `@ducanh2912/next-pwa` / Workbox-generated service worker with offline fallbacks and complete icon manifests (`public/icons/`)
...
│                                  # Web manifest is generated by src/app/manifest.ts;
│                                  # public/sw.js is build-generated/ignored by next-pwa
├── scripts/
│   ├── generate-icons.mjs        # Script to regenerate PWA icons using Sharp
│   ├── reset_test_data.sql       # Destructive SQL reset harness for manual debugging
│   ├── verify-schema.js          # DB schema verification script
│   ├── verify-rls.js             # RLS verification script
│   ├── verify-storage.js         # Storage bucket verification script
│   └── verify-triggers.js        # Trigger verification script
├── src/
│   ├── app/
│   │   ├── (auth)/login/         # Authentication page (URL: /login)
│   │   ├── (dashboard)/
│   │   │   ├── admin/users/      # Form 18: User Management (SUPER_ADMIN, MANAGER)
│   │   │   ├── audit-logs/       # Audit log list + record history detail routes
│   │   │   ├── delivery/verify/  # Form 14: Receiving & Inspection
│   │   │   ├── jo/               # Forms 1–4: Job Orders (new, track, queue, escalated)
│   │   │   ├── mrs/              # Forms 5,7–9: Requisition creation, manager, canvass, ledger (Form 6 retired)
...
│   │   ├── manifest.ts           # PWA Web Manifest route
...
│   ├── lib/
│   │   ├── access-control.ts     # Route/nav role matrix
│   │   ├── actions/              # Next.js Server Actions + action-results.ts wrapper
│   │   ├── audit/                # Audit taxonomy, visibility and dual-write service
│   │   ├── status-machines.ts    # Workflow status machines and role gates
...
│   └── proxy.ts                  # Next.js 16 proxy handling RBAC & session cookie sync
├── supabase/
│   ├── migrations/               # SQL migrations + read-only verify scripts
│   └── tests/migration-harness.mjs # Embedded local PostgreSQL migration replay/harness
```
...
- **Form 12 (`/transmittals/front-desk`):** Front Desk revolving cash float disbursement, COD package arrival/barcode acknowledgement, and FD float receipt acknowledgement (B4 / 0021).
...
# Run the embedded PostgreSQL migration/guard harness
(cd supabase/tests && npm test)

# Run production build validation (Webpack + PWA). In offline/egress-limited
# sandboxes this can fail while next/font fetches Geist from Google Fonts.
npm run build
...
- **Strict MRS flow chain (0012, 2026-09-18):**
  - The canonical chain is now enforced at **three layers** — app status
    ...
    PURCHASING → (IN_TRANSIT) → FULFILLED → CLOSED`, plus the
    Emergency Fast-Track bypass (no transmittal, Plan §6.A). The old Form 6
    warehouse branch (`PENDING_MANAGER → ISSUED_FROM_STOCK`) is now legacy-only
    after 0020 retired the screen and STOREKEEPER role; 0021 makes that status
    move Super-Admin-only.
  - New action gates (implemented as internal exceptions and now surfaced to client pages as structured `{ success: false, error }` results after B9):
    ...
    - `verifyCashAndMarkReceived` (Form 11): MRS must be `FULFILLED`
...
- **Session hardening — fix for "Minified React error #441" (2026-09-18):**
  ...
  - **Completed 2026-09-18 (after JO-flow report):** the same unguarded `getUser()`
    pattern is now migrated in the server action/audit/layout call sites —
    `jo-actions.ts`, `purchaser-actions.ts`, `transmittal-actions.ts`,
    `pms-actions.ts`, `audit-actions.ts`, `user-actions.ts`, `audit-service.ts`,
    and `(dashboard)/layout.tsx` use the sanctioned wrapper. `src/proxy.ts` is the
    one deliberate exception: it must call `supabase.auth.getUser()` to refresh and
    validate the session cookie, and wraps that call in try/catch so a validation
    ...
    sign in again." / login redirect. **Use `getServerUser()` in Server Components,
    Server Actions, and audit services; do not introduce new direct server-side
    `supabase.auth.getUser()` calls outside `src/proxy.ts`.**
...
- **PWA & Assets:** 7 sharp-generated PWA PNG icons in `public/icons/`. The web manifest is registered via `src/app/manifest.ts`; `public/sw.js` is generated by `next-pwa` during build and ignored by Git.
- **Build Status:** Current `npm run build` reaches the Next/PWA compilation step but fails in this sandbox because `next/font/google` cannot fetch Geist from `fonts.googleapis.com` (`ECONNRESET`). `npx tsc --noEmit`, `npm run lint`, and the embedded Supabase harness are the current green checks; no source compile/type error is known.
- **Git Branch:** this Arena session works on `arena/01a0bc51-fixflow-erp` (branched from `10cf860`). Keep all changes on this branch.
...
|---|---|---|---|
| **A1** | **CRITICAL — FIXED by 0016/0021** | **Gate B / Gate C inputs were writable by the constrained roles.** No trigger and no CHECK protected `spare_change_required`, `spare_change_returned`, `requester_verification`, `total_actual_spent`, `allocated_budget`, or `transmittal_forms.amount`; all three MRS triggers fired `BEFORE UPDATE OF overall_status` **only**. RLS `mrs_update_safe` / `transmittal_update_safe` were column-blind. **0016 now protects the gate-input columns and CHECK constraints; 0021 adds actor authority for `overall_status`.** | `0005_fix_rls_recursion.sql:105-133` (no `WITH CHECK`, no column scope) · triggers: `0013:159`, `0014:51` (both `UPDATE OF overall_status`), `0013:209`/`0014:117` (`receiver_status`), `0015:107` (`sender_status`) · `0001` had **no** `CHECK (amount > 0)` · fixes: `0016_gate_input_protection.sql`, `0021_status_authority_and_fd_float_ack.sql` |
...
| **A6** | **HIGH — FIXED in 0021 (§13.10)** | **`overall_status` had no role authority at the DB layer.** `guard_mrs_status_transition()` (0011→0013) validated the transition *chain* only — never *who* moves it — and neither 0016's Rules 1-9 nor 0020's Rules O1-O5 covered the status column. **0021 adds `guard_mrs_status_actor()`**: each status edge is tied to its owning role/form, Form 14 edges require the matching `requester_verification` stamp, no-app-writer edges are Super-Admin-only by omission, and JO-cascade `VOIDED` remains available to the JO requester/MRS requester/Maintenance/Manager/Super Admin. Harness G9 was replaced by NEG **N18–N20** and POS **P35–P39**. | `0021_status_authority_and_fd_float_ack.sql` · `0021_verify.sql` · harness 58/58 |
...
| **B4** | MEDIUM — FIXED in 0021/app (§13.10) | **The FD float legs never completed Rule 3.** `FD_REVOLVING_DISBURSEMENT` and `FD_REVOLVING_REPLENISHMENT` were inserted with `receiver_status='PENDING'` and no Form 12 acknowledgement action. Fixed with `acknowledgeFrontDeskFloat()`, a third Form 12 mode, `FD_FLOAT_ACKNOWLEDGED` audit action, and 0021's policy/trigger exemptions for Front Desk RECEIVED on only the two FD float types. | `transmittal-actions.ts` · `transmittals/front-desk/page.tsx` · `0021_status_authority_and_fd_float_ack.sql` · harness P40/P41/N21 |
...
| **B9** | MEDIUM — FIXED (§13.10) | **§10.6's "return a structured result, never throw" pattern was applied to only 1 client-called action.** Fixed by adding `runServerAction()` / `ActionResult` and wrapping client-called JO, MRS, Purchaser, Transmittal, PMS and User actions. Client pages now check `!result.success` and display `result.error`, so production users see the business-rule message instead of a React/Next digest. | `src/lib/actions/action-results.ts` · all client-called action files/pages |
...
| **C3** | LOW — FIXED by 0016 | §12.3's `requester_verification` vocabulary CHECK constraint is now present (`PENDING_DELIVERY`, `VERIFIED`, `DISPUTED`) as part of 0016's gate-input protection. | `0016_gate_input_protection.sql` · `0016_verify.sql` |
...
### 13.4 Recommended remediation (phased; source/migrations shipped through 0021)
...
1. ~~**Migration 0016 — protect the gate inputs (A1, B3, C3).**~~ ✅ **SHIPPED
   ...
   **Supabase SQL Editor application owner-reported complete on 2026-09-23 (0016 then 0017, after 0015).**
2. ~~**App authorization pass (A2, B6, B8).**~~ ✅ **SHIPPED 2026-09-20** — role gates
   ...
   **0018**. See §13.6 for the gate/route cross-check, the behaviour changes and the
   three deliberate exclusions. **0018 Supabase SQL Editor application owner-reported complete on 2026-09-23 (after 0017).**
3. ~~**Cash math (A3, B7, B10).**~~ ✅ **SHIPPED 2026-09-20** — migration **0019**
   ...
   **0019 Supabase SQL Editor application owner-reported complete on 2026-09-23 (after 0018).**
...
5. ~~**Rule 3 completion for the float (B4).**~~ ✅ **SHIPPED 2026-09-20** — Form 12
   now has an acknowledgement mode and `acknowledgeFrontDeskFloat()` marks FD float
   legs RECEIVED; migration **0021** updates the transmittal policy/guards so Front
   Desk can do this without weakening Accounting Gate B/C. See §13.10.
6. ~~**Error surfacing (B9).**~~ ✅ **SHIPPED 2026-09-20** — client-called JO, MRS,
   Purchaser, Transmittal, PMS and User actions now return structured
   `{ success, error }` results through `runServerAction()`, and pages render the
   actual business-rule message instead of a production digest. See §13.10.
7. ~~**A5 — align the MRS/transmittal row policies with the roles the app routes
   ...
   **and** the retirement of the storekeeper role, so migration **0020** widens
   ...
   removed from the app. See §13.9. **0020 Supabase SQL Editor application owner-reported complete on 2026-09-23 (after 0019).**
8. ~~**A6 — role authority for `overall_status` (0021).**~~ ✅ **SHIPPED 2026-09-20** —
   migration **0021** adds the actor matrix as `guard_mrs_status_actor()` and keeps
   JO-cascade voiding intact; Form 14 status moves must carry the matching
   `requester_verification` stamp, so status-only console PATCHes are blocked.
   See §13.10. **0021 Supabase SQL Editor application owner-reported complete on 2026-09-23 (after 0020).**

Each phase is independently shippable and must finish green on
`tsc`, `eslint`, and the relevant migration/app harness. `npm run build` should be run
where network egress allows Google Fonts (or with the documented offline-font shim), and
any build result must be recorded with the phase.
...
**Status after §13.10:** the remediation queue from §13.4 is now shipped in source/migrations:
**A4** is closed by 0017, **B3/C3** by 0016, **A2/B6/B8 + A1c's app layer** by Phase 2,
**A3/B7/B10** by Phase 3, **B1/B2/B5/C2** by Phase 4, **A5** by 0020, and
**A6/B4/B9** by 0021 + the structured-result app pass. owner reported live Supabase SQL application complete on 2026-09-23 for **0016 → 0017 → 0018 → 0019 → 0020 → 0021** in that order.
...
**Update:** the former Phase 5/6 items are now shipped in §13.10: B4 Front Desk float
acknowledgement, B9 structured Server Action results, and A6 DB actor authority for
`overall_status` via migration 0021. **A5**, the row-policy/role mismatch found in Phase 4,
was decided by the owner and shipped as migration 0020 — see §13.9.
...
3. Former Phases 5–6 are shipped in §13.10: **B4** (Front Desk float acknowledgement)
   and **B9** (structured results for client-called actions).
...
#### A6 — the finding 0020 exposed (implemented in 0021)
...
widens it from one person to the whole department. Harness **G9** reproduced it: a dept-2
STAFF moved a dept-2 requisition `IN_TRANSIT → FULFILLED`, which then let Form 14 stamp
Gate C and Form 11 close the cash chain on a trip nobody purchased.

This is now closed by **0021** (see §13.10): the per-transition actor matrix was audited
against the app writers and implemented as `guard_mrs_status_actor()`. The three legal
status-machine edges with no app writer now fall through to the Super Admin override only;
JO-cascade `VOIDED` stays available to the JO/MRS requester, MAINTENANCE, MANAGER and
SUPER_ADMIN; and Form 14 transitions require the matching `requester_verification` stamp,
which blocks the status-only PATCH that exposed A6 while preserving the real Form 14
workflow.
...
| `npm run build` | §13.9 had succeeded with a temporary offline-font shim and `layout.tsx` restored. Current §13.10 rerun fails in this sandbox while `next/font/google` fetches Geist (`fonts.googleapis.com` `ECONNRESET`), with no TypeScript/lint errors. |
| `cd supabase/tests && npm test` | §13.9 baseline: **47/47 gated cases** plus **GAP 1/1** for A6. Current §13.10 run after 0021: **58/58** · POSITIVE **35/35** · NEGATIVE **21/21** · RESIDUAL **2/2** · GAP **0/0** · `0016_verify` through **`0021_verify`** all **PASS** |
...
1. Owner reported **0020 applied in the SQL Editor after 0019** on 2026-09-23; if not already done, run `0020_verify.sql` — checks 1–9
...
3. **Applied migration order is 0016 → 0017 → 0018 → 0019 → 0020.** Re-applying 0016 after
...
4. Owner reported **0021 applied after 0020** on 2026-09-23; if not already done, run `0021_verify.sql` — checks 1–6 must all say PASS.
   The `overall_status` actor matrix and the FD float acknowledgement DB exemptions are
   now implemented; the owner no longer needs to decide a separate 0021 design.
5. Former Phases 5–6 are shipped in §13.10: **B4** (FD float acknowledgement) and **B9**
   (structured results for client-called actions).

### 13.10 Phase 5/6 + 0021 shipped — A6, B4, B9 closed (2026-09-20)

**Findings closed:** **A6** (`overall_status` role authority), **B4** (Front Desk float
acknowledgement / Rule 3 completion), and **B9** (structured Server Action results for
client-called actions). New migration **0021** + `0021_verify.sql`.

**Files changed**

| File | Change |
|---|---|
| `supabase/migrations/0021_status_authority_and_fd_float_ack.sql`, `0021_verify.sql` | **NEW** — `guard_mrs_status_actor()` + trigger, Front Desk update policy/field-guard/receipt-guard updates for FD float acknowledgement. |
| `src/lib/actions/action-results.ts` | **NEW** — `runServerAction()` wrapper returning `{ success: false, error }` instead of throwing to client components. |
| `src/lib/actions/{jo,mrs,purchaser,transmittal,pms,user}-actions.ts` | Client-called actions wrapped in structured results; original throwing bodies kept as `*Impl()` functions. |
| `src/app/(dashboard)/**` client pages | Every affected call site now checks `!result.success` and renders the returned error instead of relying on thrown Server Action errors. |
| `src/app/(dashboard)/transmittals/front-desk/page.tsx` | New Mode 3 — **Acknowledge Receipt** for pending `FD_REVOLVING_DISBURSEMENT` / `FD_REVOLVING_REPLENISHMENT` rows. |
| `src/lib/audit/audit-types.ts` | Added `FD_FLOAT_ACKNOWLEDGED` audit action. |
| `supabase/tests/migration-harness.mjs` | Applies/verifies 0021; G9 flipped into negative/positive cases; new B4 acknowledgement cases. |

**A6 design shipped.** 0021 adds `guard_mrs_status_actor()` as a second `BEFORE UPDATE OF
overall_status` trigger. The existing `guard_mrs_status_transition()` still validates the
state-machine edge; the new guard validates the actor:

- Form 7 transitions → MANAGER / SUPER_ADMIN.
- Form 8 transitions → BUDGET_OFFICER / SUPER_ADMIN.
- Form 10 first transmittal transition → BUDGET_OFFICER or ACCOUNTING / SUPER_ADMIN.
- Form 11 disburse / close transitions → ACCOUNTING / SUPER_ADMIN.
- Form 13 cash-confirm, in-transit and actuals transitions → PURCHASER / SUPER_ADMIN.
- Form 14 delivery/dispute transitions → requester's department / SUPER_ADMIN **and** the
  update must carry the matching `requester_verification` value (`VERIFIED` for
  `FULFILLED`, `DISPUTED` for `DISPUTED`). This blocks the status-only console PATCH that
  exposed A6 while preserving the real Form 14 writer.
- Form 6's retired stock path and rejected-row resets have no app writer and therefore fall
  through to the Super Admin override only.
- JO-cascade `VOIDED` remains allowed for the MRS requester, JO requester, MAINTENANCE,
  MANAGER, and SUPER_ADMIN, preserving the 0017 cascade fix.

**B4 design shipped.** `acknowledgeFrontDeskFloat()` marks the receiver side of Front Desk
float legs RECEIVED and writes `FD_FLOAT_ACKNOWLEDGED` to the audit trail. Migration 0021:

1. Adds FRONT_DESK to `transmittal_update_safe` (behind `is_active_account()`).
2. Redefines `guard_transmittal_fields()` so `receiver_status → RECEIVED` is still
   Accounting-only except for `FD_REVOLVING_DISBURSEMENT` and `FD_REVOLVING_REPLENISHMENT`,
   which Front Desk may acknowledge.
3. Redefines `guard_transmittal_receipt()` so FD float acknowledgements still require
   `sender_status IN ('SENT','RECEIVED')` but are exempt from MRS Gate B/Gate C close logic.
   Accounting settlement transmittals still go through Gate B/C unchanged.

**B9 design shipped.** The production digest problem is addressed at the action boundary:
client-called actions now return structured results via `runServerAction()` and log a JSON
server-side error with action context. Pages were updated to display `result.error`. This
covers JO, MRS, Purchaser, Transmittal, PMS and User actions; `verifyCashAndMarkReceived()`
kept its existing specialized structured wrapper.

**Verification**

- `npx tsc --noEmit` ✅ clean.
- `npm run lint` ✅ 0 errors, 1 known pre-existing warning (`mrs/page.tsx` unused
  `refreshing`).
- `cd supabase/tests && npm test` ✅ **58/58 gated cases** · POSITIVE **35/35** · NEGATIVE
  **21/21** · RESIDUAL **2/2** · GAP **0/0**; `0016_verify` through **`0021_verify`** all PASS.
- `npm run build` was attempted but blocked by the known sandbox egress issue fetching
  Google Fonts (`next/font` → `fonts.googleapis.com` `ECONNRESET`). This is the same §11.3
  offline-font limitation; `src/app/layout.tsx` was not modified.
...
### 13.11 Repo ↔ `agent_handoff.md` alignment audit (2026-09-20)

The handoff was re-audited against the checked-out repository after the B4/B9/A6 work.
This section records what was checked and the stale handoff claims corrected.

**Checked against the repo**

- `package.json` / `next.config.ts`: Next.js 16.3.5, webpack scripts,
  `@ducanh2912/next-pwa`, TypeScript 5, `html2canvas-pro` only.
- `src/app/**/page.tsx`: current route tree, including `(auth)/login`, `audit-logs`,
  Form 6 removal, and all 22 `NAV_CATALOG` routes in `src/lib/access-control.ts`.
- `src/lib/access-control.ts`: active role list, retired `STOREKEEPER`, longest-prefix
  route rules, nav catalog, and role-primary actions.
- `src/lib/actions/*-actions.ts`: structured-result wrappers now cover client-called JO,
  MRS, Purchaser, Transmittal, PMS, and User actions; `verifyCashAndMarkReceived()` keeps
  its specialized structured wrapper.
- `src/app/(dashboard)/**`: affected client pages check `!result.success` before showing
  success UI or refreshing.
- `src/lib/audit/audit-types.ts` and `transmittal-actions.ts`: `FD_FLOAT_ACKNOWLEDGED` and
  `acknowledgeFrontDeskFloat()` are present.
- `supabase/migrations/`: files 0001 through 0021 and verify scripts through
  `0021_verify.sql` are present; 0021 has the A6 status-actor guard and B4 FD float
  receipt policy/trigger exceptions.
- `supabase/tests/migration-harness.mjs`: replays migrations through 0021 and verifies the
  58-case matrix.

**Handoff mismatches found and fixed in this audit**

- Tech stack now names the actual PWA package/path (`@ducanh2912/next-pwa` with Workbox
  output) instead of the stale Serwist wording.
- Directory tree now reflects `(auth)/login`, `src/app/manifest.ts`, ignored/generated
  `public/sw.js`, `audit-logs`, `action-results.ts`, and the removed Form 6 route.
- §4 role matrix now mirrors `ROUTE_ACCESS_RULES` and notes longest-prefix matching.
- Form 12 now explicitly includes FD float receipt acknowledgement (B4/0021).
- Current-status text no longer describes Form 6 as an active branch; it is legacy-only and
  Super-Admin-only for status movement after 0021.
- Session guardrail text now records the only intentional direct server-side
  `supabase.auth.getUser()` call: `src/proxy.ts` session refresh/validation with try/catch.
- Current build/branch status now matches this Arena checkout: branch
  `arena/01a0bc51-fixflow-erp`; `npm run build` is blocked by Google Fonts egress in the
  sandbox rather than passing on this machine.
- Essential commands now include `npx tsc --noEmit`, `npm run lint`, and the embedded
  Supabase harness.

**Current validation after the alignment audit**

- `npx tsc --noEmit` ✅ passed.
- `npm run lint` ✅ passed with the known warning only:
  `src/app/(dashboard)/mrs/page.tsx:88:47` unused `refreshing`.
- `cd supabase/tests && npm test` ✅ **58/58** · POSITIVE **35/35** · NEGATIVE **21/21** ·
  RESIDUAL **2/2** · GAP **0/0**; `0016_verify` through `0021_verify` all PASS.
- `npm run build` ❌ still fails only while fetching Geist through `next/font/google` from
  `fonts.googleapis.com` (`ECONNRESET` after 3 retries). No TypeScript/lint/source compile
  failure was found.

**Known non-repo-state caveat**
...
- `0018_verify.sql` — checks 1–2 PASS; checks 3–4 INFO.
- `0019_verify.sql` — checks 1–4 PASS; checks 5–6 INFO.
- `0020_verify.sql` — checks 1–9 PASS; checks 10–11 INFO.
- `0021_verify.sql` — checks 1–6 PASS.
