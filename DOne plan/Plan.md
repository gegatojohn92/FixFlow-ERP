...
- [x] 1.2 Configure Storage buckets: `site-photos`, `item-references`, `receipts-proofs`, `messenger-snapshots`.
- [x] 1.3 Run `0002_rls_policies.sql` (§3.3) — **every table gets a policy before it ships**, not just the two named in the original docs.
- [x] 1.4 Run `0003_reference_numbering.sql` (§3.4) and `0004_cascade_triggers.sql` (§3.5), including the illegal-transition trigger called out under §3.5.
